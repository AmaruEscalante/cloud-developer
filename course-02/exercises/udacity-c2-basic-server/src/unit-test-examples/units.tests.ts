...
describe('concat', () => {
  ...
  it('should concat two strings', () => {
    const result = concat("hola ", "amigo");
    expect(result).to.equal("hola amigo");
  });
  
  it('it should throw an error if one of the strings is empty', () => {
    expect(()=> {concat("hola ", "")}).to.throw('Empty strings');
  });
